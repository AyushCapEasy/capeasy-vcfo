...
- [ ] **M2 — Auth + multi-tenant + roles + shell + audit log.** STATUS: _partial · 2026-06-08 — **Next.js app scaffolded** (Next 16.2.7 + React 19 + Tailwind v4, `src/` layout, `@/*` alias) and merged into the repo preserving DB tooling/migrations/env. Typed Supabase substrate in place: `@supabase/ssr` server + browser clients (ANON-key, RLS-gated; service_role never in any request path) typed against the generated `Database`; session-refresh wired via the Next 16 **proxy** convention (`src/proxy.ts` → `lib/supabase/session.ts`). `database.types.ts` relocated to `src/lib/`. `npm run build` / `typecheck` / `lint` all GREEN. **Still pending:** login/sign-out routes + auth gating, org(=client) workspace + role (Admin/Analyst) gating, app shell/nav, audit-log writes + viewer._
