...
### D-014 · D-007 REVERTED — real data permitted in `capeasy-vcfo` for IN-MEMORY testing only; data-separation DEFERRED but a MANDATORY, un-droppable launch blocker
- **Date:** 2026-06-11 (revised 2026-06-20 to match actual practice). · **Milestone:** M-Zoho (first real integration) + Tally Route-C (Orafor reconciliation). · **Operator-directed.** Sources: `vCFO-Zoho-Integration-Spec.md`, `vCFO-Tally-Integration-Design.md`.
- **Supersedes D-007's "fake data only" rule** (D-007 had said `capeasy-vcfo` is PERMANENTLY fake Acme/Globex only). That is now **lifted, narrowly**: **real data — own-firm OR a client's — is permitted in `capeasy-vcfo` for IN-MEMORY TESTING ONLY**, so an integration can be built and proven against real numbers (KISS — build/prove, then secure).
- **Hard limits on "real" (what we actually did, and the only thing allowed):** real data is read **in-memory only**, from **gitignored paths** (`.client-data.local/`), with **NO DB persistence, no writes to any tracked path, and nothing committed**. Proven in practice: the **Orafor** client Tally export was reconciled entirely in-memory (gitignored, never persisted, never committed); the **own-firm Zoho** pull is read-only. Bulk/multi-client storage is **NOT** permitted here — that waits for data-separation.
- **🔴 UN-DROPPABLE LAUNCH BLOCKER (deferred, NOT gone):** before **any client-facing launch** — or ANY persistence of real client data — it MUST live in a **SEPARATE, deliberately-provisioned, RLS-security-reviewed production project**, not this dev project. **Deferred, not removed**; tracked and cleared before launch. "It works in capeasy-vcfo" must never become the reason to ship/persist client data from here.
...
- **Reversible?** The launch blocker and the no-persistence rule are **not** reversible-away — they are the deliberate gate. The dev-project "real data in-memory" relaxation is intentional and scoped to pre-launch build/prove work.
