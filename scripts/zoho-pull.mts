...
if (!accounts.length) {
  console.log('\n--- classification coverage: SKIPPED — no account names available (see endpoint errors above) ---');
} else {
  const { rows, coverage: c } = classifyZohoAccounts(accounts);
  console.log(`\n--- classification coverage: REAL account names [source: ${accountSource}] → decision engine (Stage 1 + gate) ---`);
